            ...
            <ul class="pagination">
                ...
                if ($total_pages > 1) {
                    if($current_page - 1 >= 1) {
                        ...
                        echo "<li><a href='posts_by_category.php?q=". urlencode($q) . "&current_page={$per_page}&post_category_id={$post_category_id}'>&laquo;</a></li>";
                    }
                    for($i=1; $i <= $total_pages; $i++) {
                        if ($i == $current_page) {
                            echo "<li class='active'><a href='posts_by_category.php?q=". urlencode($q) . "&current_page={$i}&post_category_id={$post_category_id}'>{$i}</a></li>";
                        } else {
                            echo "<li><a href='posts_by_category.php?q=". urlencode($q) . "&current_page={$i}&post_category_id={$post_category_id}'>{$i}</a></li>";
                        }
                    }
                    if(($current_page + 1) <= $total_pages) {
                        $next_page = $current_page + 1;
                        echo "<li><a href='posts_by_category.php?q=". urlencode($q) . "&current_page={$next_page}&post_category_id={$post_category_id}'>&raquo;</a></li>";
                    }
                }
                ?>
            </ul>
